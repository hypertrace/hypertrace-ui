...
export class UserTelemetryInternalService {
  ...
  private readonly initializeAction$: Observable<TelemetryAction>;
  ...
  public constructor(private readonly injector: Injector, private readonly router: Router) {
    this.initializeAction$ = this.telemetryAction$.pipe(
      filter(action => action.type === TelemetryActionType.Initialize)
    );
  ...
  public initialize(): void {
    this.telemetryActionSubject.next({ type: TelemetryActionType.Initialize });
  }
  ...
  private setupInitializeHandler(): void {
    this.initializeAction$.subscribe(_ =>
