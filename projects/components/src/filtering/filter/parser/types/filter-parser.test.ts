...
describe('Filter Parser', () => {
  ...
  test('correctly parses CONTAINS_KEY with dot', () => {
    ...
    const parsedFilter: ParsedFilter<unknown> | undefined = containsFilterParser.parseFilterString(attribute, filterString);
  ...
  test('correctly parses CONTAINS_KEY_VALUE for tag with dot and URL', () => {
    const attribute: FilterAttribute = getTestFilterAttribute(FilterAttributeType.StringMap);
    const filterString: string = 'String Map Attribute.http.url CONTAINS_KEY_VALUE http://dataservice:9394/userreview?productId=f2620500-8b55-4fab-b7a2-fe8af6f5ae24'
    ...
    expect(parsedFilter).toEqual({
      ...
      value: [
        'http.url',
        'http://dataservice:9394/userreview?productId=f2620500-8b55-4fab-b7a2-fe8af6f5ae24'
      ]
    });
  });
