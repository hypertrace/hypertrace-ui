...
export class TopologyDataSourceModelPropertiesService {
  ...
  public getAllNodeCategories(): TopologyMetricCategoryData[] {
    return [this.nodeMetrics?.primary, this.nodeMetrics?.secondary, this.nodeMetrics?.others]
      .flat()
      .map(categoryData => categoryData?.categories ?? [])
      .flat();
  }

  public getAllEdgeCategories(): TopologyMetricCategoryData[] {
    return [this.edgeMetrics?.primary, this.edgeMetrics?.secondary, this.edgeMetrics?.others]
      .flat()
      .map(categoryData => categoryData?.categories ?? [])
      .flat();
  }
