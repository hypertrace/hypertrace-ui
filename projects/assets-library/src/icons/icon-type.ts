export const enum IconType {
  // These values come from Material. We will replace these with our own icon library in the future.
  Add = 'add',
  AddCircle = 'add_circle',
  AddCircleOutline = 'add_circle_outline',
  Alarm = 'svg:alarm',
  Alert = 'svg:alert',
  Application = 'device_hub',
  ArrowDown = 'svg:arrow-down',
  ArrowDownLeft = 'svg:arrow-down-left',
  ArrowDownRight = 'svg:arrow-down-right',
  ArrowLeft = 'svg:arrow-left',
  ArrowRight = 'svg:arrow-right',
  ArrowUp = 'svg:arrow-up',
  ArrowUpLeft = 'svg:arrow-up-left',
  ArrowUpRight = 'svg:arrow-up-right',
  BrowserChrome = 'svg:browser-chrome',
  BrowserFirefox = 'svg:browser-firefox',
  BrowserEdge = 'svg:browser-edge',
  BrowserInternetExplorer = 'svg:browser-ie',
  BrowserSafari = 'svg:browser-safari',
  Calls = 'svg:calls',
  Calendar = 'svg:calendar-dates',
  Cancel = 'cancel',
  CheckCircle = 'svg:check-circle',
  CheckCircleFill = 'svg:check-circle-fill',
  Checkmark = 'checkmark',
  ChevronDown = 'svg:chevron-down',
  ChevronLeft = 'svg:chevron-left',
  ChevronRight = 'svg:chevron-right',
  ChevronUp = 'svg:chevron-up',
  Clear = 'clear',
  Close = 'svg:close',
  CloseCircle = 'svg:close-circle',
  CloseCircleFilled = 'svg:close-circle-filled',
  CollapseAll = 'svg:collapse-all',
  Collapsed = 'svg:plus-square',
  Compare = 'svg:compare',
  CompareArrows = 'compare_arrows',
  ContentCopy = 'content_copy',
  Code = 'svg:code',
  Custom = 'svg:custom',
  CustomWebhook = 'svg:custom-webhook',
  Dashboard = 'svg:dashboard',
  Debug = 'bug_report',
  Delete = 'delete',
  Device = 'svg:device',
  Done = 'done',
  Download = 'download',
  DrilldownFilter = 'svg:drilldown-filter',
  Edge = 'edge',
  Error = 'error_outline',
  Empty = 'panorama_fish_eye',
  Expand = 'launch',
  ExpandAll = 'svg:expand-all',
  Expanded = 'svg:minus-square',
  ExpandedReverse = 'svg:minus-circle-reverse',
  External = 'svg:external',
  ExternalApi = 'svg:external-api',
  Eye = 'svg:eye',
  Favorite = 'favorite_border',
  FileCode = 'svg:file-code',
  Filter = 'svg:filter',
  First = 'first_page',
  Go = 'svg:go',
  Helm = 'svg:helm',
  Home = 'home',
  Hypertrace = 'svg:hypertrace',
  IpAddress = 'svg:ip-address',
  Istio = 'svg:istio',
  Info = 'svg:info',
  Infrastructure = 'svg:infrastructure',
  Internal = 'svg:internal',
  InternalApi = 'svg:internal-api',
  Java = 'svg:java',
  KeyboardArrowLeft = 'keyboard_arrow_left',
  KeyboardBackspace = 'keyboard_backspace',
  KnowledgeGraph = 'svg:knowledge-graph',
  Kong = 'svg:kong',
  Kubernetes = 'svg:kubernetes',
  Label = 'label',
  Last = 'last_page',
  Layers = 'layers',
  LightBulb = 'svg:light-bulb',
  LinkUrl = 'svg:link-url',
  List = 'format_list_bulleted',
  Loading = 'svg:loading',
  Lock = 'lock',
  MoreHorizontal = 'svg:more-horizontal',
  MoreVertical = 'svg:more-vertical',
  Menu = 'menu',
  Next = 'navigate_next',
  NoData = 'svg:noData',
  Node = 'node',
  None = 'none',
  Note = 'svg:note',
  OpenInNewTab = 'svg:open-in-new-tab',
  PageView = 'pageview',
  Previous = 'navigate_before',
  Protocol = 'svg:protocol',
  Python = 'svg:python',
  Refresh = 'refresh',
  Remove = 'remove',
  RemoveCircle = 'remove_circle',
  Restore = 'restore',
  Search = 'svg:search',
  Settings = 'settings',
  Share = 'share',
  SidebarCollapse = 'svg:sidebar-collapse',
  SidebarExpand = 'svg:sidebar-expand',
  SidebarLeft = 'svg:sidebar-left',
  Slack = 'svg:slack',
  Sort = 'svg:sort',
  SpanId = 'bookmark_border',
  Spinner = 'svg:spinner',
  StatusCode = 'svg:status-code',
  Time = 'schedule',
  TraceId = 'svg:trace-id',
  TriangleDown = 'svg:triangle-down',
  TriangleLeft = 'svg:triangle-left',
  TriangleRight = 'svg:triangle-right',
  TriangleUp = 'svg:triangle-up',
  User = 'person_outline',
  Unavailable = 'remove_circle_outline',
  Unknown = 'device_unknown',
  Warning = 'svg:warning',
  VirtualMachine = 'svg:virtual-machine',
  Yaml = 'svg:yaml',
  ZoomToFit = 'zoom_out_map'
}
